...
TRACE_EVENT(plsc_open,
	TP_PROTO(long long unsigned start, long long unsigned delta, char* tmp, int fd, struct kstat* stat_struct_ptr, int flags, umode_t mode, int error, int session),
	TP_ARGS(start, delta, tmp, fd, stat_struct_ptr, flags, mode, error, session),
	TP_STRUCT__entry(
		__field(long long unsigned, start)
		__field(long long unsigned, delta)
		__field(long, uid)
		__field(long, pid)
	...
	TP_printk("{\"start\":%llu, \"delta\":%llu, \"uid\":%lu, \"pid\":%lu, \"path\":\"%s\", \"fd\":%i, \"size\":%lli, \"type\": %i, \"flags\":%i, \"mode\":%i, \"ERR\":%i, \"session\":%i}", __entry->start, __entry->delta, __entry->uid, __entry->pid, __entry->pathname, __entry->fd, __entry->size, __entry->type, __entry->flags, __entry->mode, __entry->error, __entry->session)
);
...
TRACE_EVENT(plsc_read,
	TP_PROTO(long long unsigned start, long long unsigned delta, ssize_t ret, int session, unsigned int fd, size_t count, loff_t pos_old),
	TP_ARGS(start, delta, ret, session, fd, count, pos_old),
	TP_STRUCT__entry(
		__field(long long unsigned, start)
		__field(long long unsigned, delta)
	...
	TP_printk("{\"start\":%llu, \"delta\":%llu, \"error\":%i, \"session\":%i, \"fd\":%i, \"bytes\":%i, \"offset\":%llu}", __entry->start, __entry->delta, __entry->error, __entry->session, __entry->fd, __entry->bytes, __entry->offset)
);
